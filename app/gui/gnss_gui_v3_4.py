...
class GNSSMainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("GNSS Processing GUI v3.4")
        ...
        self.log_model = ProcessingLog(version="0.3.4")
    ...
    def load_file(self):
        try:
            ...
            self.log("Импорт завершён успешно.")
            self.plot_layers()
    ...
    def run_median(self):
        try:
            ...
            out_df = filt.apply(df, time_col=time_col, height_col=height_col)
            self.df_med = out_df
            self.log(
                f"Медианный фильтр применён к слою {stage}: "
                f"окно={cfg.window_points}, порог={cfg.threshold_mm} мм"
            )
            self.plot_layers()
    ...
    def plot_layers(self):
        try:
            ...
            def plot_df(df, label, color):
                ...
                y = pd.to_numeric(df[height_col], errors="coerce")
            ...
            if self.cb_raw.isChecked():
                plot_df(self.df_raw, "Исходный", "C0")
            if self.cb_valid.isChecked():
                plot_df(self.df_validated, "После проверки времени", "C1")
            if self.cb_clean.isChecked():
                plot_df(self.df_clean, "После очистки", "C2")
            if self.cb_bw.isChecked():
                plot_df(self.df_bw, "Баттерворт", "C3")
            if self.cb_med.isChecked():
                plot_df(self.df_med, "Медианный", "C4")

            self.canvas.ax.set_title(self.current_file.name if self.current_file else "GNSS график")
            self.canvas.ax.set_xlabel("Время")
            self.canvas.ax.set_ylabel(
                "Высота, м" if self.y_mode == "absolute_m" else "Отклонение от среднего, мм"
            )
    ...
    def save_current_csv(self):
        try:
            ...
            out_path, _ = QFileDialog.getSaveFileName(
                self, "Сохранить CSV", str(default_name), "CSV files (*.csv)"
            )
            if not out_path:
                return
    ...
    def save_current_jpg(self):
        try:
            ...
            out_path, _ = QFileDialog.getSaveFileName(
                self, "Сохранить график JPG", str(default_name), "JPG files (*.jpg)"
            )
            if not out_path:
                return
