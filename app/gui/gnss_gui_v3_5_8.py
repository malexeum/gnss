...
class GNSSMainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle(f"GNSS Processing GUI v{self.APP_VERSION}")
        ...
        self.log_model = ProcessingLog(version=self.LOG_VERSION)
    ...
    def _auto_guess_columns(self, df: pd.DataFrame):
        ...
        time_candidates = [
            "utc_time_utc", "utc_time", "datetime_utc", "datetime utc", "datetime",
            "utc time", "датаивремя", "датавремя", "дата_и_время", "date_time",
            "time_utc", "gpst", "utc", "date", "datetimeutc", "Дата-Время",
        ]

        height_candidates = [
            "height_m", "height", "высотам", "высота", "Высота (м)", "h",
            "h_ellips_m", "H_ellips_m", "hellipsm", "ellipsoidal_height", "ellips_height",
        ]
    ...
    def _sync_range_editors(self):
        ...
        self.range_from_edit.setDateTime(QDateTime(t_min.year, t_min.month, t_min.day, t_min.hour, t_min.minute, t_min.second))
        self.range_to_edit.setDateTime(QDateTime(t_max.year, t_max.month, t_max.day, t_max.hour, t_max.minute, t_max.second))
    ...
    def load_file(self):
        try:
            ...
            self._set_status("Импорт данных...", "work")
            self.log("=== Импорт ===")
            ...
            if "utc_time" in df_raw.columns:
                t_all = pd.to_datetime(df_raw["utc_time"], errors="coerce", utc=True).dt.tz_localize(None).dropna()
                if len(t_all) > 0:
            ...
            self.log_step("import", "Импорт завершён успешно", {"rows": len(df_raw), "time_col": self.time_col, "height_col": self.height_col})
            self._sync_range_editors()
            self.plot_layers()
            self._set_status(f"Импорт завершён: {len(df_raw)} строк.", "done")
        except Exception as e:
            QMessageBox.critical(self, "Ошибка импорта", str(e))
            self.log_model.add_error(str(e), module="import")
            self.log(f"Ошибка импорта: {e}")
            self._set_status("Ошибка импорта.", "error")
    ...
    def validate_time(self):
        try:
            ...
            self._set_status("Проверка времени...", "work")
            ...
            self._sync_range_editors()
            self.plot_layers()
            self._set_status(f"Проверка времени завершена, N={len(self.df_validated)}.", "done")
        except Exception as e:
            ...
            self._set_status("Ошибка проверки времени.", "error")
    ...
    def run_outlier(self):
        try:
            ...
            self._set_status("Очистка выбросов...", "work")
            ...
            self._sync_range_editors()
            self.plot_layers()
            self._set_status(f"Очистка выбросов завершена, осталось {len(clean)} точек.", "done")
        except Exception as e:
            ...
            self._set_status("Ошибка очистки выбросов.", "error")
    ...
    def run_butterworth(self):
        try:
            ...
            self._set_status("Фильтр Баттерворта...", "work")
            ...
            self._sync_range_editors()
            self.plot_layers()
            self._set_status(f"Баттерворт завершён, N={len(self.df_bw)}.", "done")
        except Exception as e:
            ...
            self._set_status("Ошибка фильтра Баттерворта.", "error")
    ...
    def run_median(self):
        try:
            ...
            self._set_status("Медианный фильтр...", "work")
            ...
            self._sync_range_editors()
            self.plot_layers()
            self._set_status(f"Медианный фильтр завершён, N={len(self.df_med)}.", "done")
        except Exception as e:
            ...
            self._set_status("Ошибка медианного фильтра.", "error")
    ...
    @staticmethod
    def _series_stats_text(label: str, series: pd.Series, unit_mode: str):
        s = pd.to_numeric(series, errors="coerce").dropna()
        if len(s) == 0:
            return None
        std_val = s.std(ddof=1) if len(s) > 1 else 0.0
        if unit_mode == "m":
            return f"{label}: N={len(s)} ср={s.mean():.4f} м СКО={std_val * 1000.0:.2f} мм min={s.min():.4f} max={s.max():.4f}"
        return f"{label}: N={len(s)} ср={s.mean():.2f} мм СКО={std_val:.2f} мм min={s.min():.2f} max={s.max():.2f}"

    def _build_stats_lines_for_selection(self, series_map: list[tuple[str, pd.Series, str]]) -> str:
        lines = []
        for label, series, unit_mode in series_map:
            txt = self._series_stats_text(label, series, unit_mode)
            if txt:
                lines.append(txt)
        return "\n".join(lines)
    ...
    def plot_layers(self):
        try:
            ...
            def plot_df(df, label, color, y_col=None, linewidth=1.0, alpha=1.0, stats_unit="m"):
                nonlocal plotted
                if df is None or df.empty:
                    return
            ...
            stats_text = ""
            if plotted_series:
                x_left_num, x_right_num = ax.get_xlim()
                x_left_dt = mdates.num2date(x_left_num).replace(tzinfo=None)
                x_right_dt = mdates.num2date(x_right_num).replace(tzinfo=None)
                stats_map = []
                for label, df_plot, unit_mode in plotted_series:
                    mask = (df_plot["t"] >= x_left_dt) & (df_plot["t"] <= x_right_dt)
                    df_window = df_plot.loc[mask]
                    if not df_window.empty:
                        stats_map.append((label, df_window["y"], unit_mode))
                stats_text = self._build_stats_lines_for_selection(stats_map)
    ...
    def save_current_csv(self):
        try:
            ...
            out_path, _ = QFileDialog.getSaveFileName(self, "Сохранить CSV", str(default_path), "CSV files (*.csv)")
            if not out_path:
                return
    ...
    def save_current_jpg(self):
        try:
            ...
            out_path, _ = QFileDialog.getSaveFileName(self, "Сохранить график JPG", str(default_path), "JPG files (*.jpg)")
            if not out_path:
                return
    ...
    def save_log(self):
        try:
            ...
            out_path, _ = QFileDialog.getSaveFileName(self, "Сохранить лог", str(default_path), "Text files (*.txt);;JSON files (*.json)")
            if not out_path:
                return
