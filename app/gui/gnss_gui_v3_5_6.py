...
from PySide6.QtWidgets import (
    ...
    QGroupBox, QCheckBox, QComboBox, QDateTimeEdit,
)
...
class GNSSMainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("GNSS Processing GUI v3.5.6")
        ...
        self.log_model = ProcessingLog(version="0.3.5.6")
        ...
        def add_stats(label: str, col: str, unit_mode: str):
            ...
            if unit_mode == "m":
                ...
                lines.append(
                    ...
                    f"СКО={std_val * 1000.0:.2f} мм min={s.min():.4f} max={s.max():.4f}"
                )
            else:
                std_val = s.std(ddof=1) if len(s) > 1 else 0.0
                lines.append(
                    f"{label}: N={len(s)} ср={s.mean():.2f} мм "
                    f"СКО={std_val:.2f} мм min={s.min():.2f} max={s.max():.2f}"
                )
    ...
    def plot_layers(self):
        try:
            ...
            if stats_source is not None:
                time_col = "utc_time" if "utc_time" in stats_source.columns else self.time_col
                if time_col and time_col in stats_source.columns:
                    ...
                    t_all = pd.to_datetime(stats_source[time_col], errors="coerce", utc=True).dt.tz_localize(None)
                    mask = t_all.notna() & (t_all >= x_left_dt) & (t_all <= x_right_dt)
                    df_window = stats_source.loc[mask].copy()
                else:
                    df_window = stats_source.copy()

            if plotted:
                ax.legend(loc="upper left")

            if df_window is not None and not df_window.empty:
                txt = self._stats_text_from_df(df_window)
                if txt:
                    ax.text(
                        0.995,
                        0.995,
