...
class GNSSMainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("GNSS Processing GUI v3.5")
        ...
        self.log_model = ProcessingLog(version="0.3.5")
    ...
    def _build_ui(self):
        ...
        for btn in [
            ...
            btn_plot, btn_save_csv, btn_save_jpg, btn_save_log
        ]:
    ...
    def log_step(self, module: str, summary: str, params=None, level: str = "INFO"):
        self.log_model.add(module=module, summary=summary, params=params or {}, level=level)
        self.log(summary)

    def _base_output_dir(self) -> Path:
        if self.current_dir is not None:
            base = self.current_dir / "output"
        elif self.current_file is not None:
            base = self.current_file.parent / "output"
        else:
    ...
    def choose_file(self):
        start_dir = str(self.current_dir) if self.current_dir else ""
        ...
        self.log_step("gui", f"Выбран файл: {self.current_file}", {"file": str(self.current_file)})
    ...
    def _auto_guess_columns(self, df: pd.DataFrame):
        ...
        time_candidates = [
            ...
            "Дата-Время", "Time_UTC", "GPST", "UTC", "Date"
        ]
        height_candidates = [
    ...
    def load_file(self):
        try:
            ...
            self.log(f"Размер таблицы: {len(df_raw)} строк, {len(df_raw.columns)} столбцов")
            self.log(f"Колонки: {list(df_raw.columns)}")
            ...
            if manual_time and manual_height:
                ...
                if not time_col:
                    raise ValueError(
                        "Автоопределение колонок не сработало: "
                        f"Не найдена временная колонка. Столбцы: {list(df_raw.columns)}"
                    )
                if not height_col:
                    raise ValueError(
                        "Автоопределение колонок не сработало: "
                        f"Не найдена колонка высоты. Столбцы: {list(df_raw.columns)}"
                    )
            ...
            if self.time_col not in df_raw.columns:
                raise ValueError(
                    f"Указанная колонка времени '{self.time_col}' не найдена. "
                    f"Доступные столбцы: {list(df_raw.columns)}"
                )
            if self.height_col not in df_raw.columns:
                raise ValueError(
                    f"Указанная колонка высоты '{self.height_col}' не найдена. "
                    f"Доступные столбцы: {list(df_raw.columns)}"
                )
    ...
    def validate_time(self):
        try:
            ...
            report_text = normalized.report.to_text()
            self.log(report_text)
            self.log_step(
                "time_validator",
                "Проверка времени завершена.",
                {
                    "time_col": self.time_col,
                    "height_col": self.height_col,
                    "rows_out": len(self.df_validated),
                },
            )
            self.plot_layers()
    ...
    def run_butterworth(self):
        try:
            ...
            self.log_step(
                "butterworth",
                f"Баттерворт применён к слою {stage}.",
                {
                    "stage": stage,
                    "time_col": time_col,
                    "height_col": height_col,
    ...
    def run_median(self):
        try:
            ...
            out_df = filt.apply(df)
            self.df_med = out_df

            summary = MedianResidualFilter.summary(out_df)
            self.log_step(
                "median_filter",
                f"Медианный фильтр применён к слою {stage}.",
                {
                    "stage": stage,
    ...
    def _median_live_recalc(self):
        if not self.cb_median_live.isChecked():
            return
        if self.df_clean is None and self.df_bw is None:
            return
        try:
            self.run_median()
        except Exception:
    ...
    def _on_y_mode_changed(self, idx: int):
        text = self.y_mode_combo.currentText()
        self.y_mode = "absolute_m" if "Абсолютная" in text else "mean_mm"
        self.plot_layers()
    ...
    def plot_layers(self):
        try:
            ...
            def plot_df(df, label, color, y_col=None, residual=False):
                nonlocal plotted, plotted_residual, stats_source
                ...
                col = y_col if y_col else ("height" if "height" in df.columns else self.height_col)
                if col not in df.columns:
                    return
                ...
                if len(t) == 0:
                    return
                if self.y_mode == "mean_mm" and not residual:
                    y = self._to_mm_relative(y)
                ax.plot(t, y, label=label, color=color, linewidth=0.9)
                plotted = True
                plotted_residual = plotted_residual or residual
                if stats_source is None:
                    stats_source = df

            if self.cb_raw.isChecked():
                plot_df(self.df_raw, "Исходный", "C0", y_col="height" if self.df_raw is not None and "height" in self.df_raw.columns else self.height_col)
            if self.cb_valid.isChecked():
                plot_df(self.df_validated, "После проверки времени", "C1", y_col="height" if self.df_validated is not None and "height" in self.df_validated.columns else self.height_col)
            if self.cb_clean.isChecked():
                plot_df(self.df_clean, "После очистки", "C2", y_col="height")
            if self.cb_bw.isChecked():
                if self.y_mode == "mean_mm" and self.df_bw is not None and "residual_bw_mm" in self.df_bw.columns:
                    plot_df(self.df_bw, "Баттерворт", "C3", y_col="height_bw", residual=False)
                else:
                    plot_df(self.df_bw, "Баттерворт", "C3", y_col="height_bw")
            if self.cb_med.isChecked():
                plot_df(self.df_med, "Медианный", "C4", y_col="height_med")
            ...
            if stats_source is not None:
                stats_text = self._stats_text_from_df(stats_source)
                if stats_text:
                    ax.text(
                        0.995,
                        0.995,
    ...
    def save_current_csv(self):
        try:
            ...
            out_path, _ = QFileDialog.getSaveFileName(
                self, "Сохранить CSV", str(default_path), "CSV files (*.csv)"
            )
            if not out_path:
                return
            df.to_csv(out_path, index=False, encoding="utf-8-sig")
            self.log_step("export_csv", f"Сохранена таблица {stage}: {out_path}", {"stage": stage, "path": out_path})
        except Exception as e:
    ...
    def save_current_jpg(self):
        try:
            ...
            out_path, _ = QFileDialog.getSaveFileName(
                self, "Сохранить график JPG", str(default_path), "JPG files (*.jpg)"
            )
            if not out_path:
                return
            ...
            self.log_step("export_jpg", f"График сохранён в JPG: {out_path}", {"path": out_path})
        except Exception as e:
    ...
    def save_log(self):
        try:
            ...
            if suffix == ".json":
                ...
                if suffix != ".txt":
                    out_path = str(Path(out_path).with_suffix(".txt"))
                Path(out_path).write_text(self.log_model.to_text(), encoding="utf-8")
            self.log_step("export_log", f"Лог сохранён: {out_path}", {"path": out_path})
        except Exception as e:
