...
class GNSSMainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("GNSS Processing GUI v3.5.2")
        ...
        self.log_model = ProcessingLog(version="0.3.5.2")
    ...
    def _base_output_dir(self) -> Path:
        base = (self.current_dir or Path.cwd()) / "output"
    ...
    def choose_file(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(self, "Выбрать GNSS файл", start_dir, "Data files (*.csv *.txt *.tsv *.xls *.xlsx *.pos);;All files (*.*)")
        if not file_path:
            return
    ...
    def validate_time(self):
        try:
            if self.df_raw is None:
                ...
                return
            normalized = self.time_validator.validate(raw_table=self.importer._raw_table, time_col=self.time_col, height_col=self.height_col, date_col=None)
    ...
    def run_outlier(self):
        try:
            ...
            otl = OutlierDetector(time_col=time_col, height_col=height_col, k_sigma=self.k_sigma_spin.value(), window_sec=self.window_sec_spin.value(), min_group=self.min_group_spin.value(), max_group_epochs=self.max_group_spin.value())
            clean, removed, flagged, report = otl.run(self.df_validated)
            self.df_clean = clean
            self.df_removed = removed
            self.df_flagged = flagged
            self.log(str(report))
            self.log_step("outlier_detector", "Очистка выбросов завершена", {"rows_clean": len(clean) if clean is not None else 0, "rows_removed": len(removed) if removed is not None else 0, "k_sigma": self.k_sigma_spin.value(), "window_sec": self.window_sec_spin.value()})
            self.plot_layers()
        except Exception as e:
    ...
    def run_butterworth(self):
        try:
            ...
            butterworth_on_csv(input_csv=str(tmp_in), output_csv=str(tmp_out), time_col=time_col, height_col=height_col, period_minutes=period_min, order=order)
            ...
            self.log_step("butterworth", "Баттерворт применён", {"stage": stage, "order": order, "period_min": period_min, "rows_out": len(self.df_bw), "tmp_out": str(tmp_out)})
    ...
    def run_median(self):
        try:
            ...
            cfg = MedianFilterConfig(window_points=int(self.window_points_spin.value()), threshold_mm=float(self.threshold_mm_spin.value()))
            self.df_med = MedianResidualFilter(cfg).apply(base_df)
            s = MedianResidualFilter.summary(self.df_med)
            self.log_step("median_filter", "Медианный фильтр применён", {"stage": base_stage, "window_points": cfg.window_points, "threshold_mm": cfg.threshold_mm, **s})
            self.plot_layers()
        except Exception as e:
    ...
    def plot_layers(self):
        try:
            ...
            if self.cb_clean.isChecked():
                plot_df(self.df_clean, "После очистки", "C2", "height")
            if self.cb_bw.isChecked():
                plot_df(self.df_bw, "Баттерворт", "C3", "height_bw")
            if self.cb_med.isChecked():
                plot_df(self.df_med, "Медианный", "C4", "height_med")
